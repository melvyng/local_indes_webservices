...
Web service plugin for Moodle 4.X
------------------------------------------
...
- indes_webservices_get_nameless_grades_participant_category : Get the enrolled & dropout participants (without firstname & lastname) with their grade from a category (courses), including user status
- indes_webservices_get_nameless_grades_participants_by_category : Get the enrolled & dropout participants (without firstname & lastname) with their grade from a category (courses) without user status
- indes_webservices_kpi_get_nameless_grades_participant_category : Get the enrolled & dropout participants (without firstname & lastname) with their grade from a category (courses), not including user status
- indes_webservices_kpi_basic_grades_participant_category : Get the enrolled & dropout participants IDs, progress and user status
...
- indes_webservices_update_bigbluebuttonbn_recordings : Update the status of a specific recording in the table mdl_bigbluebuttonbn_recordings
- indes_webservices_update_bigbluebuttonbn_recordingids : Update the recordingid of a specific recording in the table mdl_bigbluebuttonbn_recordings
- indes_webservices_delete_bigbluebuttonbn_recordings : Delete a specific recording in the table mdl_bigbluebuttonbn_recordings